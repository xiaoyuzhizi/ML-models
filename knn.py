...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    import math
    xn=x_train.view(num_train,-1)
    xt=x_test.view(num_test,-1)
    #print("x_train:",xn,"\n")
    #print("x_test:",xt,"\n")
    num=xt.shape[1]
    for i in range(0,num_train):
      for j in range(0,num_test):
        dists[i][j]=((xn[i]-xt[j])**2).sum()**(1/2)
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    #print("xt: ",xt.size())
    AB2=xn.mm(xt.t())*2
    a2s=(xn**2).sum(dim=1).reshape(-1,1)
    print("a2s shape:",a2s.size())
    b2s=(xt**2).sum(dim=1).t()
    dists=(a2s-AB2+b2s)**(1/2)
    

    #print("xn-xt: ",(xn-xt).size())
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    value,indice=torch.topk(dists,k,largest=False,dim=0)
    print("value:",value)
    print("indice:",indice)

    for i in range(num_test):
      aaa,idx=torch.max(y_train[indice[:,i]].bincount(),dim=0)
      y_pred[i]=idx
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_cross_validate(
    ...
    for k in k_choices:
        for i in range(num_folds):
            ...
            y_train_local=[x for x in y_train_folds]
            x_test=x_train_folds[i]
            y_test=y_train_folds[i]
            del x_train_local[i]
            del y_train_local[i]
            x_train = torch.cat(x_train_local, dim=0)
            y_train = torch.cat(y_train_local, dim=0)
            knn=KnnClassifier(x_train,y_train) 
            k_to_accuracies[k].append(knn.check_accuracy(x_test,y_test,k))
    """
    for k in k_choices:
    list_of_acc = []
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies.keys():
        cur=sum(k_to_accuracies[k])/len(k_to_accuracies[k])
        print(cur)
        if cur>maxa:
            best_k=k
            maxa=cur
